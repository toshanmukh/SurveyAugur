    ...
    public String saveShareUrl(MailDetails mailDetails, UUID surveyId) throws ServiceException {
       ...
       try{
           if(activeSurveysRepository.findBySurveyId(surveyId) != null)
           {
               ...
               message=message+" Survey Title: "+activeSurveys.getSurveyTitle()+" .The link to survey is: "+activeSurveys.getSurveyUrl();
               for(String email:emailIds) {
                   mqSender.send(eventObject, request, message, "Survey Invite",email);
               }
               return "Url Sent";
           }
           else
